...
exports.leaveReviews = async function (req,res) {
    try{
        ...
        if (!review) {
            return res.status(404).json({message: 'The review could not be added!'});
        }
        ...
        if(!product)
            return res.status(404).json({message: 'Internal Service Error'});
