...
exports.addToWishlist = async function (req,res) {
    try{
        ...
        const productAlreadyExists = user.wishlist.find((item)=> item.productId.equals(new mongoose.Schema.Types.ObjectId(req.body.productId)));
        if(productAlreadyExists) {
            return res.status(409).json({ message: 'Product already exist in wishlist' });
        }
